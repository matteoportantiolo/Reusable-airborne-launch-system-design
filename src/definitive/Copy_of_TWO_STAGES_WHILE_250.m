...
% 1) First case
% mass_payload = 400 [kg]
% x_opt_previous = [2.3441794872244267e+02, 2.3156252810693908e-04, 1.1341514180166154e+04, 1.4694739604793995e+02, 1.9587770396019306e+00, ...
% 4.1405118475544947e+02, 6.0447743490349082e-01, 3.2385257125780591e+01, 2.7026472665545693e+03];
% 
% feval_opt_previous = 1.351934131691041e+02;
% 
%  The error in orbit insertion velocity is : +2.09069e+01 [m/s] 
% 
%  The error in final flight path angle is : +2.28504e-03 [rad] 
% 
%  The error in final altitude is : +3.47028e-02 [m] 
%

% --------------------------------------------------------------------------------------------------
% 2) Second case
% mass_payload = 250 [kg]
% x_opt_previous =  [1.8925167025497197e+02, 3.9272582296130600e-02, 1.1328365962702101e+04,...
%                    1.4037088400688259e+02, 5.8921817474518123e+00, 4.0666639858744543e+02,...
%                    6.3036437650682797e-01, 3.1340723096772731e+01, 2.3841529939073685e+03]; 
% feval_opt_previous =   39.273700718044381;
% 
% The error in orbit insertion velocity is : +2.31381e+01 [m/s] 
% 
% The error in final flight path angle is : +2.79728e-05 [rad] 
% 
% The error in final altitude is : +1.47370e+01 [m] 
%
% Stage 1 parameters
% stage1.D = 1.6;            % Diameter [m]
% stage1.A = pi * (stage1.D / 2)^2; % Cross-sectional area [m^2]
% stage1.Cd = 0.2;           % Drag coefficient
% stage1.Cl = 0;             % Lift coefficient
% stage1.m0 = +23959-150;         % Initial mass [kg]
% stage1.I_sp = 323.16;      % Specific impulse [s]
% stage1.m_prop = 18494 ;     % Propellant mass [kg]
% stage1.TW = 1.6;           % Thrust-to-weight ratio
...
% stage1.gamma_rate = deg2rad(40)/stage1.t_burn; % Imposed gamma rate [rad/s]
% stage1.epsilon = 0.12;
% 
% % Stage 2 parameters
% stage2.D = 1.3;            % Diameter [m]
% stage2.A = pi * (stage2.D / 2)^2; % Cross-sectional area [m^2]
% stage2.Cd = 0.2;           % Drag coefficient
% stage2.Cl = 0;             % Lift coefficient
% stage2.m0 =  2543 + 400-150;    % Initial mass [kg] including payload
% stage2.I_sp = 336.58;      % Specific impulse [s]
% stage2.m_prop = 2187;      % Propellant mass [kg]
% stage2.TW = 1.35;           % Thrust-to-weight ratio
...
stage1.Cd = 0.2;           % Drag coefficient
stage1.Cl = 0;             % Lift coefficient
stage1.m0 = 14974.323707+m_add;         % Initial mass [kg]
stage1.I_sp = 323;      % Specific impulse [s]
stage1.m_prop = 11558.734135 + m_add*(1-stage1.epsilon) ;     % Propellant mass [kg]
stage1.TW = 1.6;           % Thrust-to-weight ratio
stage1.delta = 0;          % Thrust vector angle [rad]
stage1.T = stage1.m0 * 9.81 * stage1.TW; % Thrust [N]
stage1.t_burn = stage1.m_prop * stage1.I_sp * 9.81 / stage1.T;
stage1.gamma_rate = deg2rad(38)/stage1.t_burn; % Imposed gamma rate [rad/s]
...
stage2.Cd = 0.2;           % Drag coefficient
stage2.Cl = 0;             % Lift coefficient
stage2.m0 =  1589.398553+250;    % Initial mass [kg] including payload
stage2.I_sp = 336;      % Specific impulse [s]
stage2.m_prop = 1366.882756;      % Propellant mass [kg]
stage2.TW = 1.35;           % Thrust-to-weight ratio
stage2.delta = 0;          % Thrust vector angle [rad]
stage2.T = stage2.m0 * 9.81 * stage2.TW; % Thrust [N]
stage2.t_burn = stage2.m_prop * stage2.I_sp * 9.81 / stage2.T;
...
% Check delle masse 
%[MASS, m_struct,m_prop] = OptimalStagingFun(stage1.epsilon, stage2.epsilon, stage1.I_sp, stage2.I_sp, 9.17e3 , 250)
...
gamma = 1.35;              % Specific heat ratio (adiabatic index)
...
% lb = [180, deg2rad(0.001),      9e3,        100,            95,        stage2.t_burn-110,            deg2rad(5)]; % Lower bounds
% ub = [v_c, deg2rad(15),         12e3,       stage1.t_burn,  300,         stage2.t_burn+100,  deg2rad(30)]; % Upper bounds
lb = [180,  deg2rad(0.001),      9e3,        1,            0.1,        stage2.t_burn-30,      deg2rad(1),         10, 0]; % Lower bounds
ub = [v_c,  deg2rad(15),         12e3,       stage1.t_burn,  700,      stage2.t_burn+300,  deg2rad(37.5),        stage2.t_burn-5, 10000]; % Upper bounds

% ========================================================================
%% OPTIMIZATION
% ========================================================================
% [v0, gamma0, h0, t_bo1, delta_t, t_bo2, delta_gamma, t_thrott, extra prop mass in 2nd stage]
%initial_guess = [v_c-10, deg2rad(5), 11e3, stage1.t_burn-10, 90, stage2.t_burn+10, deg2rad(25)]; 


% !!!!!!! PREVIOUS BEST RESULT !!!!!
% %________________________________________________________________________________________________________________________________
%  x_opt_previous =  [1.8000000000000000e+02, 5.6755373487293469e-04, 1.0746905353026465e+04,...
%                     1.4710827803078530e+02, 1.6009438057342159e+01, 4.1507907296847361e+02, ...
%                     6.3849427261303193e-01, 3.2447049292847929e+01, 1.5382234328554753e+03]; 
% feval_opt_previous =   1.360644381375409e+02;
% 

% x_opt_previous =  [2.1461181343046013e+02, 3.9323417870970002e-03, 1.0848878544118743e+04,...
%                     1.4536647452532131e+02, 2.6318637326057104e+00, 4.2160161565702333e+02, ...
%                     6.1076120302774839e-01, 3.3564229729702753e+01, 1.7116314324155665e+03];
% 
% feval_opt_previous = 66.679045052795743;
% 
% m_add = 3000;

x_opt_previous =  1.0e+04.*[0.022611912988013   0.000003102454507   1.081632308501518   0.014471058818199   0.000481038388898 0.041586019805905   0.000058004079705   0.003370003592815   0.168359400703143];

feval_opt_previous = 13.783708616623354;


% %_________________________________________________________________________________________________________________________________


%feval_opt_previous = 1e4;

counter = 0 ;
x_opt = NaN(1,9);
feval_opt = 1e6;
N_max = 10;
%initial_guess0 = [1.83e+02, deg2rad(4), 11e3, 100, 10, 210, deg2rad(25),30,2900];

% Old guess 
% initial_guess0 = 1e4*[ 0.021918120486018,   0.000001026065378,   1.107907544896800,   0.014603448423855,   0.000668225818079,...
%                  0.041468470042340, 0.000060256486548,   0.003095190966241,   0.256439680779262];

% initial_guess0 = [1.8000000000000000e+02, 5.6755373487293469e-04, 1.0746905353026465e+04,...
%                     1.4710827803078530e+02, 1.6009438057342159e+01, 4.1507907296847361e+02, ...
%                     6.3849427261303193e-01, 3.2447049292847929e+01, 1.5382234328554753e+03]; 

% initial_guess0 = [2.1461181343046013e+02, 3.9323417870970002e-03, 1.0848878544118743e+04,...
%                     1.4536647452532131e+02, 2.6318637326057104e+00, 4.2160161565702333e+02, ...
%                     6.1076120302774839e-01, 3.3564229729702753e+01, 1.7116314324155665e+03];

% initial_guess0 = 1.0e+04.*[0.018724032147710   0.000001427692043   1.085574452526715   0.014537393752009   0.001091163782567 0.042175977383460   0.000060503556346   0.003361628718006   0.162717884143634];

initial_guess0 =  1.0e+04.*[0.022611912988013   0.000003102454507   1.081632308501518   0.014471058818199   0.000481038388898 0.041586019805905   0.000058004079705   0.003370003592815   0.168359400703143];


while counter <= N_max && feval_opt>1e2

        initial_guess = initial_guess0 ...
                        + rand(1,9)*0.01.*initial_guess0 -0.005.*initial_guess0;
        ...
        % Run optimization
        [x_temp, feval_temp] = fmincon(@(x) ObjFun(x, stage1, stage2), ...
            initial_guess, [], [], [], [], lb, ub, ...
            @(x) myconstraints(x, stage1, stage2), options);


        if feval_temp< feval_opt_previous
            feval_opt_previous = feval_temp;
            x_opt_previous = x_temp;
        end
        counter = counter+1
end
...
%% RESULTS DISPLAY
% ========================================================================
fprintf(' \nOptimized error insertion: %+.5e [m/s]\n', feval_opt);

x_opt = x_opt_previous;
...
delta_gamma = x_opt(7);      % Delta angle given by the pitch up manouvre [rad]
t_throttling = x_opt(8);
plus_mass = x_opt(9); 
...
fprintf([' \nOptimized initial state:\n' ...
    ...
    'Burnout time 2: %+.5e [s]\n'...
    'Delta gamma : %+.5e [deg]\n'...
    'Additional fuel on the second stage : %.5e [kg]\n'], ...
    v_opt, gamma_opt, h_opt, t_bo1_opt, delta_t_opt, t_bo2_opt,rad2deg(delta_gamma),plus_mass);
...
options_pu = odeset('reltol', 1e-7, 'abstol', 1e-7, 'Events', @(t,y) Karman_line(t, y, true)); 
options = odeset('reltol', 1e-7, 'abstol', 1e-7);  % Options for the other phases without event function

% Step 1: Integrate the trajectory for the pitch-up phase
[tt1, xx1] = ode78(@(t,x) fun_pitchup(t,x,stage1,stage2,t_bo1_opt,delta_gamma,plus_mass), [0 t_bo1_opt], xx_0_NEW, options_pu); 
...
[tt_off, xx_off] = ode78(@(t,x) fun_off(t,x,stage2,plus_mass), [0 delta_t_opt], xx1(end,:), options);
...
[tt2, xx2] = ode78(@(t,x) fun(t,x,stage2,t_throttling,plus_mass), [0 t_bo2_opt], xx_off(end,:), options);
% [tt2, xx2] = ode78(@(t,x) fun(t,x,stage2), [0 8000], xx_off(end,:), options);
...
title('Flight Path Angle in time', 'Interpreter', 'latex');
grid on;
...
title('Altitude in time ', 'Interpreter', 'latex');
grid on;
...
xlabel('Time [s]', 'Interpreter', 'latex', 'FontSize', 14);
ylabel('X [deg]', 'Interpreter', 'latex', 'FontSize', 14);
title('Spherical angle covered in time ', 'Interpreter', 'latex');
grid on;
...
stage2_mod.m_prop =     stage2.m_prop + plus_mass; % Initial propellant mass [kg]
stage2_mod_m_struct =   stage2_mod.m_prop*(stage2.epsilon/(1-stage2.epsilon));
stage2_mod.m0 =         +250 +stage2_mod.m_prop + stage2_mod_m_struct;
stage2_mod.t_burn =     stage2_mod.m_prop * stage2.I_sp * 9.81 / stage2.T;
stage1_mod.m0 =         stage1.m0 -stage2.m0 + stage2_mod.m0 ;
...
Cb1 = stage1_mod.m0 / (stage1.A * stage1.Cd);  % For stage 1
Cb_off = stage2_mod.m0 / (stage2.A * stage2.Cd);  % For coasting phase
Cb2 = stage2_mod.m0 / (stage2.A * stage2.Cd);  % For stage 2
...
m1 = @(t) stage1_mod.m0 - stage1.T ./ (stage1.I_sp * 9.81) .* t;  % Stage 1 mass
m2 = @(t) (t<=stage2_mod.t_burn).*(stage2_mod.m0 - stage2.T ./ (stage2.I_sp * 9.81) .* t)...
          +(t>stage2_mod.t_burn).*(stage2_mod.m0 );  % Stage 2 mass
...
term1_d = -0.5 ./ Cb1 .* rho(h_vect1) .* v_vect1 .^ 2 .* stage1_mod.m0 ./ m1(tt1);  % Drag during pitch-up
term_off_d = -0.5 ./ Cb_off .* rho(h_vect_off) .* v_vect_off .^ 2;  % Drag during coasting
term2_d = -0.5 ./ Cb2 .* rho(h_vect2) .* v_vect2 .^ 2 .* stage2_mod.m0 ./ m2(tt2);  % Drag during stage 2
...
if  t_bo2_opt>=stage2_mod.t_burn
    m2_left=0;
else
    m2_left = stage2.T / (stage2.I_sp * 9.81) * (stage2_mod.t_burn - t_bo2_opt);
end

perc1 = m1_left / stage1.m_prop * 100;
perc2 = m2_left / stage2_mod.m_prop * 100;

fprintf('\nRemaining Propellant Mass:\n');
fprintf('  - Stage 1: %.2f [kg] (%.2f%%)\n', m1_left, perc1);
fprintf('  - Stage 2: %.2f [kg] (%.2f%%)\n', m2_left, perc2);

% Visualising the re-entry contitions in terms of mass
fprintf('\nRemaining Mass of the 1 stage at staging condition :\n');
fprintf('  - Stage 1 re-entry condition: %.2f [kg] \n', m1(t_bo1_opt)-m2(0));
%%
% Recovery
m_recovery = m1(t_bo1_opt)-m2(0);
dv_recovery = 9.81*323*log(m_recovery/(m_recovery-m1_left));
fprintf('\nDeltaV for re-entry : %.2f [m/s]\n',dv_recovery);
...
% Launch site position depends on where do we want to release the launcher
% from the aircraft to respect all constraints related to sfe operations
...
% 
% 
% long = long0 + xx_combined(:,4)*cos(inc0);
% long = wrapTo180(rad2deg(long));
% lat = lat0 + xx_combined(:,4)*sin(inc0);
...
% 
 ...
 test = 2.344179487227887e+02.*[cos(deg2rad(98)); sin(deg2rad(98))] - 463.13344*cos(deg2rad(60)).*[1;0]
...
function [dydt] = fun(t, y, param,t_throttling,plus_mass)
    % Unpack state variables
    v = y(1); % Velocity [m/s]
    gamma = y(2); % Flight path angle [rad]
    h = y(3); % Altitude [m]

    m_prop0 =     param.m_prop + plus_mass; % Initial propellant mass [kg]
    m_struct =   m_prop0*(param.epsilon/(1-param.epsilon));
    m0 =         +250 +m_prop0 + m_struct;
    %t_burn =     m_prop0 * param.I_sp * 9.81 / param.T;
    ...
    %m0 = param.m0 + (1.14)*plus_mass; % Initial mass [kg] ------>   1.14 to account for the extra propellant in the stage and 
    %                                                               its impact on the structural mass
    I_sp = param.I_sp; % Specific impulse [s]
    %m_prop0 = param.m_prop+plus_mass; % Initial propellant mass adjucted with an extra contribution [kg]
    delta = param.delta; % Thrust angle (unused since T=0) [rad]
    trottling = param.trottling; % Time of throttling event ( should be irrelevant code with no effective throttling)
    ...
    if ~isempty(trottling) && t > t_throttling && nargin>3
        T = trottling*T;
    end
...
function [dydt] = fun_pitchup(t, y, param1,param2,t_bo1,delta_gamma,plus_mass)
    ...
    % Unpack parameters
    T = param1.T; % Thrust [N]
    A = param1.A; % Cross-sectional area [m^2]
    Cd = param1.Cd; % Drag coefficient
    %m0 = param1.m0+plus_mass*1.14; % Initial mass + additional mass on the second stage [kg]
    I_sp = param1.I_sp; % Specific impulse [s]

    m_prop2 =     param2.m_prop + plus_mass; % Initial propellant mass [kg]
    m_struct2 =   m_prop2*(param2.epsilon/(1-param2.epsilon));
    m02 =         +250 +m_prop2 + m_struct2;
    %t_burn =     m_prop2 * param2.I_sp * 9.81 / param2.T;
    m0 =         param1.m0 -param2.m0 + m02 ;

    gamma_rate = delta_gamma/t_bo1; % Rate of change of flight path angle [rad/s]
    %gamma_rate = param.gamma_rate; % Rate of change of flight path angle [rad/s]
    m_prop0 = param1.m_prop; % Initial propellant mass [kg]
    delta = param1.delta; % Thrust angle (unused since T=0) [rad]
...
function [dydt] = fun_off(~, y, param,plus_mass)
    ...
    Cd = param.Cd; % Drag coefficient
    Cl = param.Cl; % Lift coefficient
    %m0 = param.m0+plus_mass*1.14; % Total mass + additional mass on the second stage(structure + prop) [kg]
    delta = param.delta; % Thrust angle (unused since T=0) [rad]

    m_prop2 =     param.m_prop + plus_mass; % Initial propellant mass [kg]
    m_struct2 =   m_prop2*(param.epsilon/(1-param.epsilon));
    m0 =         +250 +m_prop2 + m_struct2;
    %t_burn =     m_prop2 * param2.I_sp * 9.81 / param2.T;
...
function [F, G] = ObjFun(x_vect, param_1st, param_2st)
    ...
    delta_gamma =x_vect(7); % Final value of the flight path angle in the first leg of the trajectory
    t_throttling = x_vect(8);
    plus_mass = x_vect(9);
    ...
    [tt1, xx1] = ode78(@(t, x) fun_pitchup(t, x, param_1st,param_2st,t_bo1,delta_gamma,plus_mass), [0 t_bo1], y0, options_pu);
    [tt_off, xx_off] = ode78(@(t, x) fun_off(t, x, param_2st,plus_mass), [0 delta_t], xx1(end, :), options);
    [tt2, xx2] = ode78(@(t, x) fun(t, x, param_2st,t_throttling,plus_mass), [0 t_bo2], xx_off(end, :), options);
    ...
    F = abs(v_vect2(end) -sqrt(mu / (r_f_target / 1e3))*1e3 ) + abs(gamma_vect2(end))*5e4 + abs(h_vect2(end)-400e3);
   
    % Gradient of the objective function (optional, not impemented in the near future)
    if nargout > 1
...
function [c, ceq, G, Geq] = myconstraints(x_vect, param_1st, param_2st)
    ...
    options_pu = odeset('reltol', 1e-5, 'abstol', 1e-6, 'Events', @(t, y) Karman_line(t, y, true));
    options = odeset('reltol', 1e-5, 'abstol', 1e-6);
    [~, xx1] = ode78(@(t, x) fun_pitchup(t, x, param_1st, param_2st,t_bo1,delta_gamma,plus_mass), [0 t_bo1], y0, options_pu);
    [~, xx_off] = ode78(@(t, x) fun_off(t, x, param_2st,plus_mass), [0 delta_t], xx1(end, :), options);
    [~, xx2] = ode78(@(t, x) fun(t, x, param_2st,t_throttling,plus_mass), [0 t_bo2], xx_off(end, :), options);
    ...
    % Inequality constraints (currently the oneson the first stage separation point)
    % c = [];
    c(1) = xx1(end,1)-3e3;
    c(2) = xx1(end,3)-90e3;
   
    % c(3) = xx1(end,2)-deg2rad(40);
    % c(4) = -(xx1(end,2)-deg2rad(30));
...
function [value, isterminal, direction] = Karman_line(~, y, isTerminal)
    ...
    value = h - 130e3; % Event condition: trigger when altitude exceeds 130 km
