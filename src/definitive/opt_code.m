...
DV = 9.75*1e3;
mpay = 250;
[MASS, m_struct,m_prop] = OptimalStagingFun(eps1, eps2, isp1, isp2, DV, mpay);
...
% Stage 1 parameters
stage1.D = 1.5;            % Diameter [m]
stage1.A = pi * (stage1.D / 2)^2; % Cross-sectional area [m^2]
stage1.Cd = 0.5;           % Drag coefficient
stage1.Cl = 0;             % Lift coefficient
stage1.m0 = MASS.glom;         % Initial mass [kg]
stage1.I_sp = isp1;      % Specific impulse [s]
stage1.m_prop = m_prop(1);     % Propellant mass [kg]
...
stage1.gamma_rate = deg2rad(37)/stage1.t_burn; % Imposed gamma rate [rad/s]
...
stage2.m0 = MASS.m2 + mpay;    % Initial mass [kg] including payload
stage2.I_sp = isp2;      % Specific impulse [s]
stage2.m_prop = m_prop(2);      % Propellant mass [kg]
stage2.TW = 1.4;           % Thrust-to-weight ratio
...
lb = [150, deg2rad(0.001), 9e3, 0, 0, 0]; % Lower bounds
ub = [v_c+20, deg2rad(5), 12e3, stage1.t_burn, 500, stage2.t_burn]; % Upper bounds

% ========================================================================
%% OPTIMIZATION
% ========================================================================
initial_guess = [220, deg2rad(1), 11e3, stage1.t_burn-10, 200, stage2.t_burn-10]; % [v0, gamma0, h0, t_bo1, delta_t, t_bo2]
