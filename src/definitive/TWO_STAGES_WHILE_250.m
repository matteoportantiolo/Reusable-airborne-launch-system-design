...
[tt1, xx1] = ode78(@(t,x) fun_pitchup(t,x,stage1,t_bo1_opt,delta_gamma,plus_mass), [0 t_bo1_opt], xx_0_NEW, options_pu); 
...
stage1_mod.m0 = stage1.m0 +plus_mass*(1+stage2.epsilon);
stage1_mod.TW = stage1.T/stage1_mod.m0/9.81;
stage2_mod.m0 = stage2.m0 +plus_mass*(1+stage2.epsilon);
stage2_mod.TW = stage2.T/stage2_mod.m0/9.81;
...
function [dydt] = fun_pitchup(t, y, param,t_bo1,delta_gamma,plus_mass)
    ...
    A = param.A; % Cross-sectional area [m^2]
    Cd = param.Cd; % Drag coefficient
    m0 = param.m0+plus_mass*1.14; % Initial mass + additional mass on the second stage [kg]
    I_sp = param.I_sp; % Specific impulse [s]
...
function [F, G] = ObjFun(x_vect, param_1st, param_2st)
    ...
    [tt1, xx1] = ode78(@(t, x) fun_pitchup(t, x, param_1st,t_bo1,delta_gamma,plus_mass), [0 t_bo1], y0, options_pu);
...
function [c, ceq, G, Geq] = myconstraints(x_vect, param_1st, param_2st)
    ...
    options_pu = odeset('reltol', 1e-5, 'abstol', 1e-6, 'Events', @(t, y) Karman_line(t, y, true));
    options = odeset('reltol', 1e-5, 'abstol', 1e-6);
    [~, xx1] = ode78(@(t, x) fun_pitchup(t, x, param_1st,t_bo1,delta_gamma,plus_mass), [0 t_bo1], y0, options_pu);
