...
eps = [ 0.12 0.15 0.17];
% first guess mass
[~, m1, m2, m3, ms1, ms2, ms3] = Staging_optimization_3st(Isp, eps, m_pay, Dv);

% new iteration mass

mp1 = m1-ms1;
mp1 = 1*mp1;
ms1 = eps(1)*mp1/(1-eps(1));
m1 = ms1+mp1;

mp2 = m2-ms2;
mp2 = 1*mp2;
ms2 = eps(2)*mp2/(1-eps(2));
m2 = ms2+mp2;

mp3 = m3-ms3;
mp3 = 1*mp3;
ms3 = eps(3)*mp3/(1-eps(3));
m3 = ms3+mp3;
...
stage1.m0 = m0;         % Initial mass [kg]
stage1.I_sp = Isp(1);      % Specific impulse [s]
stage1.m_prop = mp1;     % Propellant mass [kg]
stage1.TW = TW(1);           % Thrust-to-weight ratio
...
stage1.gamma_rate = deg2rad(34)/stage1.t_burn; % Imposed gamma rate [rad/s]
...
stage2.m0 = m2+m3+m_pay;    % Initial mass [kg] including payload
stage2.I_sp = Isp(2);      % Specific impulse [s]
stage2.m_prop = mp2;      % Propellant mass [kg]
stage2.TW = TW(2);           % Thrust-to-weight ratio
...
% Stage 3 parameters
stage3.D = 1.3;            % Diameter [m]
stage3.A = pi * (stage3.D / 2)^2; % Cross-sectional area [m^2]
stage3.Cd = 0.5;           % Drag coefficient
stage3.Cl = 0;             % Lift coefficient
stage3.m0 = m3+m_pay;    % Initial mass [kg] including payload
stage3.I_sp = Isp(3);      % Specific impulse [s]
stage3.m_prop = mp3;      % Propellant mass [kg]
stage3.TW = TW(3);           % Thrust-to-weight ratio
stage3.delta = 0;          % Thrust vector angle [rad]
stage3.T = stage3.m0 * 9.81 * stage3.TW; % Thrust [N]
stage3.t_burn = stage3.m_prop * stage3.I_sp * 9.81 / stage3.T;
...
lb = [200, deg2rad(0.001), 9e3, 0.01, 10, 0.01]; % Lower bounds
ub = [v_c+20, deg2rad(2), 12e3, stage1.t_burn, 300, stage3.t_burn]; % Upper bounds

% ========================================================================
%% OPTIMIZATION
% ========================================================================
initial_guess = [v_c-10, deg2rad(1), 11e3, stage1.t_burn-10, 150, stage3.t_burn-10]; % [v0, gamma0, h0, t_bo1, delta_t, t_bo2]
...
% Run optimization
[x_opt, delta_v_opt] = fmincon(@(x) ObjFun(x, stage1, stage2, stage3), ...
    initial_guess, [], [], [], [], lb, ub, ...
    @(x) myconstraints(x, stage1, stage2, stage3), options);
...
t_bo3_opt = x_opt(6);        % Burnout time 3 [s]
...
fprintf([' \nOptimized initial state:\n' ...
    ...
    v_opt, gamma_opt, h_opt, t_bo1_opt, delta_t_opt);
...
% Step 3: Integrate trajectory of stage 2
[tt2, xx2] = ode78(@(t,x) fun(t,x,stage2), [0 stage2.t_burn], xx1(end,:), options);
...
[tt_off, xx_off] = ode78(@(t,x) fun_off(t,x,stage2), [0 delta_t_opt], xx2(end,:), options);
...
% Step 4: Integrate trajectory of stage 3
[tt3, xx3] = ode78(@(t,x) fun(t,x,stage3), [0 stage3.t_burn], xx_off(end,:), options);

% Display the results of the stage 3 phase
fprintf('\nResults at the end of the stage 3 phase:\n')
fprintf('Final velocity: %+.5e [m/s]\n', xx3(end, 1))
fprintf('Final flight path angle: %+.5e [deg]\n', rad2deg(xx3(end, 2)))
fprintf('Final altitude: %+.5e [m]\n', xx3(end, 3))
...
err_v = abs(xx3(end, 1) - v_insertion);  % The final velocity error (in m/s)
err_gamma = abs(xx3(end, 2) - gamma_f);  % The final velocity error (in m/s)
err_h = abs(xx3(end, 3) - h_f);  % The final velocity error (in m/s)
...
if stage1.t_burn > tt1(end)
    ...
    fprintf(' \n First stage burn out due to altitude target : delta_t = %+.5e [s] \n', (stage1.t_burn - tt1(end)))
    stage1.t_burn = tt1(end);  % Update the burn-out time based on the event function
end
...
plot(tt2 + tt1(end), xx2(:, 1), 'LineWidth', 2);
plot(tt_off + tt2(end) + tt1(end), xx_off(:, 1), 'LineWidth', 2);
plot(tt3 + tt_off(end) + tt2(end) + tt1(end), xx3(:, 1), 'LineWidth', 2);
xlabel('Time [s]', 'Interpreter', 'latex', 'FontSize', 14);
ylabel('Velocity [m/s]', 'Interpreter', 'latex', 'FontSize', 14);
title('Velocity vs. Time', 'Interpreter', 'latex');
legend(["First stage pitch-up", "Separation and coasting", "Second stage", "Third stage"], ...
    'Location', 'best', 'FontSize', 12);
grid on;

% Flight Path Angle
subplot(2, 2, 2)
plot(tt1, xx1(:, 2), 'LineWidth', 2); hold on;
plot(tt2 + tt1(end), xx2(:, 2), 'LineWidth', 2);
plot(tt_off + tt2(end) + tt1(end), xx_off(:, 2), 'LineWidth', 2);
plot(tt3 + tt_off(end) + tt2(end) + tt1(end), xx3(:, 2), 'LineWidth', 2);
xlabel('Time [s]', 'Interpreter', 'latex', 'FontSize', 14);
...
plot(tt2 + tt1(end), xx2(:, 3), 'LineWidth', 2);
plot(tt_off + tt2(end) + tt1(end), xx_off(:, 3), 'LineWidth', 2);
plot(tt3 + tt_off(end) + tt2(end) + tt1(end), xx3(:, 3), 'LineWidth', 2);
xlabel('Time [s]', 'Interpreter', 'latex', 'FontSize', 14);
...
% Angular Velocity
subplot(2, 2, 4)
plot(tt1, xx1(:, 4), 'LineWidth', 2); hold on;
plot(tt2 + tt1(end), xx2(:, 4), 'LineWidth', 2);
plot(tt_off + tt2(end) + tt1(end), xx_off(:, 4), 'LineWidth', 2);
plot(tt3 + tt_off(end) + tt2(end) + tt1(end), xx3(:, 4), 'LineWidth', 2);
xlabel('Time [s]', 'Interpreter', 'latex', 'FontSize', 14);
...
tt_combined = [tt1; tt_off + tt1(end); tt2 + tt_off(end) + tt1(end); tt3 + tt2(end) + tt_off(end) + tt1(end)];
xx_combined = [xx1; xx_off; xx2; xx3];
...
v_vect3 = xx3(:,1);        % Velocity during stage 2 propulsion
gamma_vect3 = xx3(:,2);    % Flight path angle during stage 2
h_vect3 = xx3(:,3);        % Altitude during stage 2
...
Cb3 = stage3.m0 / (stage3.A * stage3.Cd);  % For stage 3
...
m3 = @(t) stage3.m0 - stage3.T ./ (stage3.I_sp * 9.81) .* t;  % Stage 3 mass
...
term3_d = -0.5 ./ Cb3 .* rho(h_vect3) .* v_vect3 .^ 2 .* stage3.m0 ./ m3(tt3);  % Drag during stage 3
...
term3_g = -g(h_vect3) .* sin(gamma_vect3);  % Gravitational loss during stage 3

% Calculate the total drag losses by integrating over the time for each phase
delta_v_drag = abs(trapz(tt1, term1_d)) + abs(trapz(tt_off, term_off_d)) + abs(trapz(tt2, term2_d)) + abs(trapz(tt3, term3_d));

% Calculate the total gravitational losses by integrating over the time for each phase
delta_v_grav = abs(trapz(tt1, term1_g)) + abs(trapz(tt_off, term_off_g)) + abs(trapz(tt2, term2_g)) + abs(trapz(tt3, term3_g));
...
D3 = @(rho,v) 0.5.*rho.*(v.^2).*stage3.A.*stage3.Cd;
...
a3 = (stage3.T./m3(tt3) - D3(rho(h_vect3),xx3(:,1))./m3(tt3) - g(h_vect3).*sin(xx3(:,2)))./9.81;

figure('Name', 'Acceleration', 'NumberTitle', 'off');
a_combined = [a1; a_off; a2; a3];
...
max_q3 = 0.5.*rho(h_vect3).*v_vect3.^2;

figure('Name', 'Max-Q Analysis', 'NumberTitle', 'off');
q_max = [max_q1; max_q_off; max_q2; max_q3];
...
function [F, G] = ObjFun(x_vect, param_1st, param_2st, param_3st)
    ...
    %               x_vect(4) = coasting time (delta_t) [s]
    %               x_vect(5) = burn time for phase 2 (t_bo2) [s]
    %               x_vect(6) = burn time for phase 3 (t_bo3) [s]
    ...
    %   param_3st - Structure containing the second set of parameters for phase 3
    %
    % OUTPUTS:
    ...
    t_bo1 = x_vect(4);  % Stage 1 burnout [s]
    ...
    [~, xx2] = ode78(@(t, x) fun(t, x, param_2st), [0 param_2st.t_burn], xx1(end, :), options);
    [~, xx_off] = ode78(@(t, x) fun_off(t, x, param_2st), [0 delta_t], xx2(end, :), options);
    [~, xx3] = ode78(@(t, x) fun(t, x, param_3st), [0 param_3st.t_burn], xx_off(end, :), options);
    ...
    % Objective function (insertion error):
    % 1. Final velocity should match the circular orbital velocity at target radius
    err_v = sqrt(mu / (r_f_target / 1e3)) * 1e3 - v_f;  % Circular orbital velocity [m/s]
    
    % 2. Final flight path angle should be zero (horizontal orbit)
    err_gamma = gamma_f;  % Final flight path angle [rad]
    
    % 3. Final altitude should be 400 km
    err_h = h_f - 400e3;  % Target altitude [m]

    F = abs(err_v) + abs(err_gamma) + abs(err_h);
    ...
    % m3 = @(t) param_3st.m0 - param_3st.T ./ (param_3st.I_sp * 9.81) .* t; % Mass during phase 3
...
function [c, ceq, G, Geq] = myconstraints(x_vect, param_1st, param_2st, param_3st)
    ...
    % INPUTS:
    %   x_vect   - Optimization variables [v0, gamma0, h0, delta_t, t_bo2, t_bo3]
    ...
    %   param_3st - Parameters for the third phase (e.g., coasting, drag, etc.)
    %
    % OUTPUTS:
    ...
    options_pu = odeset('reltol', 1e-5, 'abstol', 1e-6, 'Events', @(t, y) Karman_line(t, y, true));
    options = odeset('reltol', 1e-5, 'abstol', 1e-6);
    [~, xx1] = ode78(@(t, x) fun_pitchup(t, x, param_1st), [0 param_1st.t_burn], y0, options_pu);
    % [~, xx_off] = ode78(@(t, x) fun_off(t, x, param_2st), [0 delta_t], xx1(end, :), options);
    % [~, xx2] = ode78(@(t, x) fun(t, x, param_2st), [0 param_2st.t_burn], xx_off(end, :), options);
    % [~, xx3] = ode78(@(t, x) fun(t, x, param_3st), [0 param_3st.t_burn], xx2(end, :), options);
    % 
    ...
    c(1) = xx1(end,1) - 3e3;
    c(2) = xx1(end,2) - deg2rad(50);
    c(3) = -xx1(end,2) - deg2rad(30);
    c(4) = xx1(end,1) - 80e3;
...
function [value, isterminal, direction] = Karman_line(~, y, isTerminal)
    ...
    value = h - 100e3; % Event condition: trigger when altitude exceeds 130 km
