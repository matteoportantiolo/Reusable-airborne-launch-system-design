...
y0_new = [y0(1:3);t_pu;t_bo1;delta_t0;t_bo2];
...
lb = [ 200, 0.001, 9e3, 0, 0, 0, 0];
ub = [ v_c+20, deg2rad(2), 12e3, t_burn1, 30, 500, t_burn2]; 
...
fprintf([' \n The optimized initial state is :\n \n initial velocity : %+.5e [m/s] \n initial pitch angle : %+.5e [deg]'  ...
    ...
    '\n burnout time2 : %+.5e [s] \n'],xx_0_NEW(1),rad2deg(xx_0_NEW(2)),xx_0_NEW(3)./1e3,t_pu_opt,t_bo1_opt,delta_t_opt,t_bo2_opt)

options_pu = odeset('reltol', 1e-5, 'abstol', 1e-6,'Events',@(t,y) h_pitchup(t,y,true));
options_sep = odeset('reltol', 1e-5, 'abstol', 1e-6,'Events',@(t,y) h_separation(t,y,true));
options = odeset('reltol', 1e-5, 'abstol', 1e-6);
[tt_pu, xx_pu] =      ode78(@(t,x) fun_pitchup(t,x,param_1st), [0 t_pu_opt], xx_0_NEW,options_pu);
[tt1, xx1] =      ode78(@(t,x) fun(t,x,param_1st), [t_pu_opt t_pu_opt+t_bo1_opt], xx_pu(end,:),options_sep);
...
figure(2)
subplot(2,2,1)
plot(tt_pu,xx_pu(:,1),'LineWidth',2)
hold on
...
ylabel('v [m/s]','Interpreter','latex',FontSize=14)
title("Variation of velocity in time ",'Interpreter','latex')
legend(["First stage pitch-up","First stage continuation","Separation and coasting","Second stage"],'Location','best',FontSize=14)
grid on

subplot(2,2,2)
plot(tt_pu,xx_pu(:,2),'LineWidth',2)
hold on
plot(tt1,xx1(:,2),'LineWidth',2)
...
subplot(2,2,3)
plot(tt_pu,xx_pu(:,3),'LineWidth',2)
hold on
...
subplot(2,2,4)
plot(tt_pu,xx_pu(:,4),'LineWidth',2)
hold on
...
tt_update = [tt_pu;tt1;tt_off + tt1(end);tt2 + tt_off(end) + tt1(end)];
xx_update = [xx_pu;xx1;xx_off;xx2];
...
Cb_pu = param_1st.m0/(param_1st.A*param_1st.Cd);
...
term_pu_d = -0.5./Cb_pu.*rho(h_vect_pu).*v_vect_pu.^2.*param_1st.m0./m1(tt_pu);
...
term_pu_g = -g(h_vect_pu).*sin(gamma_vect_pu);
term1_g = -g(h_vect1).*sin(gamma_vect1);
term_off_g = -g(h_vect_off).*sin(gamma_vect_off);
term2_g = -g(h_vect2).*sin(gamma_vect2);

delta_v_drag = abs(trapz(tt_pu,term_pu_d)) + abs(trapz(tt1,term1_d)) + abs(trapz(tt_off,term_off_d)) + abs(trapz(tt2,term2_d));
delta_v_grav = abs(trapz(tt_pu,term_pu_g)) + abs(trapz(tt1,term1_g)) + abs(trapz(tt_off,term_off_g)) + abs(trapz(tt2,term2_g));
...
D_pu = @(rho,v) 0.5.*rho.*(v.^2).*param_1st.A.*param_1st.Cd;
...
a_pu = (param_1st.T./m1(tt_pu) - D_pu(rho(h_vect_pu),xx_pu(:,1))./m1(tt_pu) - g(h_vect_pu).*sin(xx_pu(:,2)))./9.81;
...
% Objective function to which is subject of a constrained minimization
    ...
    options_sep = odeset('reltol', 1e-5, 'abstol', 1e-6,'Events',@(t,y) h_separation(t,y,true));
    options = odeset('reltol', 1e-5, 'abstol', 1e-6);
    [tt_pu, xx_pu] =      ode78(@(t,x) fun_pitchup(t,x,param_1st), [0 t_pu], y0,options_pu);
    [tt1, xx1] =      ode78(@(t,x) fun(t,x,param_1st), [t_pu t_pu+t_bo1], xx_pu(end,:),options_sep);
...
% Non linear contraint function
    ...
    options_sep = odeset('reltol', 1e-5, 'abstol', 1e-6,'Events',@(t,y) h_separation(t,y,true));
    options = odeset('reltol', 1e-5, 'abstol', 1e-6);
    [~, xx_pu] =      ode78(@(t,x) fun(t,x,param_1st), [0 t_pu], y0,options_pu);
    [~, xx1] =      ode78(@(t,x) fun_pitchup(t,x,param_1st), [t_pu t_pu+t_bo1], xx_pu(end,:),options_sep);
...
function [value, isterminal, direction] = h_separation(~, y, isTerminal)
    h = y(3);
...
function [value, isterminal, direction] = h_pitchup(~, y, isTerminal)
    h = y(3);
    value = h - 80e3;    % stop when h = 100e3
    isterminal = isTerminal;       % stop integration
    direction = 0;        % direction
end
