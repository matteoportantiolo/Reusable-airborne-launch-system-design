...
lb = [200, deg2rad(0.001), 9e3, 0.01, 10]; % Lower bounds
ub = [v_c+20, deg2rad(2), 12e3, stage1.t_burn, 300]; % Upper bounds

% ========================================================================
%% OPTIMIZATION
% ========================================================================
initial_guess = [v_c-10, deg2rad(1), 11e3, stage1.t_burn-10, 150]; % [v0, gamma0, h0, t_bo1, delta_t, t_bo2]
...
[tt2, xx2] = ode78(@(t,x) fun(t,x,stage2), [0 stage2.t_burn], xx_off(end,:), options);
...
[tt3, xx3] = ode78(@(t,x) fun(t,x,stage3), [0 stage3.t_burn], xx2(end,:), options);
...
plot(tt3 + tt2(end) + tt_off(end) + tt1(end), xx3(:, 1), 'LineWidth', 2);
...
plot(tt3 + tt2(end) + tt_off(end) + tt1(end), rad2deg(xx3(:, 2)), 'LineWidth', 2);
xlabel('Time [s]', 'Interpreter', 'latex', 'FontSize', 14);
...
plot(tt3 + tt2(end) + tt_off(end) + tt1(end), xx3(:, 3), 'LineWidth', 2);
xlabel('Time [s]', 'Interpreter', 'latex', 'FontSize', 14);
...
plot(tt3 + tt2(end) + tt_off(end) + tt1(end), rad2deg(xx3(:, 4)), 'LineWidth', 2);
xlabel('Time [s]', 'Interpreter', 'latex', 'FontSize', 14);
